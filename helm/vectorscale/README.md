...
### 1. Create namespace
```bash
kubectl create namespace vectorscale
```

### 2. Install chart
```bash
helm install vectorscale ./helm/vectorscale \
  --namespace vectorscale \
  --set api.image.repository=ghcr.io/<your-username>/vectorscale-api \
  --set sidecar.image.repository=ghcr.io/<your-username>/vectorscale-sidecar
```

### 3. Verify deployment
```bash
# Check pods
kubectl get pods -n vectorscale

# Check services
kubectl get svc -n vectorscale

# Check HPA
kubectl get hpa -n vectorscale
```

### 4. Access services
```bash
# Get API LoadBalancer IP
export API_URL=$(kubectl get svc vectorscale-api -n vectorscale -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
...
# Get Jaeger UI IP
export JAEGER_URL=$(kubectl get svc vectorscale-jaeger -n vectorscale -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
...
```bash
helm install vectorscale ./helm/vectorscale \
  -f helm/vectorscale/values-prod.yaml \
  -n vectorscale
```
...
```bash
helm upgrade vectorscale ./helm/vectorscale \
  --namespace vectorscale \
...
```bash
helm uninstall vectorscale -n vectorscale
kubectl delete namespace vectorscale
```
...
### Pods not starting
```bash
kubectl describe pod <pod-name> -n vectorscale
kubectl logs <pod-name> -n vectorscale
```

### API can't reach Sidecar
```bash
# Check Sidecar service DNS
kubectl exec -it <api-pod> -n vectorscale -- curl http://vectorscale-sidecar:50051

# Check logs
kubectl logs <sidecar-pod> -n vectorscale
```

### PVC not mounting
```bash
kubectl get pvc -n vectorscale
kubectl describe pvc vectorscale-sidecar-pvc -n vectorscale
```
...
# Check HPA status
kubectl describe hpa vectorscale-api-hpa -n vectorscale
```
